...
# co chu va font cho text trong rect and cir
RectButtonFont = pygame.font.Font(myFont,16)
CirButtonFont = pygame.font.Font(myFont,40)
...
v_cells = 30  #do dai cua 1 hang va 1 cot
h_cells = 30
...
#khoi tao button de chon thuat toan
dijkstra_button = CirButton(
                            center=(17*PADDING+10,3*PADDING), radius=RADIUS, text="DJ",
...
dfs_button = CirButton(
                            center=(17*PADDING+10,12*PADDING), radius=RADIUS, text="D",
                            textcolor=colors["black"], circolor=colors["white"],
                            screen=screen, font=CirButtonFont)
...
ERASE = False
RESET = False
ALGO = None # co bao hieu cho thuat 

#board la the hien duoc tao cho bang
...
while True:
    ...
    if not SEARCH:
        ...
        if left == 1:
            ...
            if dijkstra_button.distance(mouse) < RADIUS:
                ...
                dfs_button.color_change(colors["white"])
                time.sleep(0.1)   
            ...
            # button for dfs
            elif dfs_button.distance(mouse) < RADIUS:
                ALGO = "DFS"
            ...
            for i in range(v_cells):
                for j in range(h_cells):
                    ...
                    if cell.collidepoint(mouse):
                        # neu click chuot phai ma vi tri do khong nam trong wall va diem bat dau = none
                        if(i,j) not in board.wall and board.start is None:
                            board.start = (i,j)
                        #neu (i.j) ko nam trong tuong va diem bat dau da co
    ...
    # neu seach = true
        ...
        if board.start is None or board.target is None:
            print("chon diem bat dau va ket thuc")
            SEARCH = False
            start_button.color_change(colors["white"])
            continue
        elif ALGO is None:
            print("chua chon thuat toan")
            SEARCH = False
            start_button.color_change(colors["white"])
            continue

        #set up lai duong di cu
